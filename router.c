...
int setup_tunnel (void) {
	...
	strncpy (ifreq.ifr_name, "6bed4_anycast", IFNAMSIZ);
 ...
 */
void handle_6bed4_router_advertise (void) {
...
void handle_4to6_nd (ssize_t v4ngbcmdlen) {
	...
	case ND_ROUTER_SOLICIT:
		...
		//
		// Having accepted the Router Solicitation, respond
		handle_6bed4_router_advertise ();
		break;
...
/*
 * Pass incoming IPv6 data over to IPv4.
 */
void handle_6to4_plain_unicast (ssize_t v6datalen, struct in6_addr *ip6) {
...
printf ("Received plain unicast IPv6 data, flags=0x%04x, proto=0x%04x\n", v6tuncmd.flags, v6tuncmd.proto);
	...
	handle_6to4_plain_unicast (rawlen - sizeof (struct tun_pi), v6dst6);
}
